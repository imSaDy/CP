...
// g(n, m, x):
// Counts the number of integer arrays a[1..n] such that
//   0 ≤ a[i] ≤ x
//   and a[1] + a[2] + ... + a[n] = m
// Computed using stars and bars with inclusion–exclusion.

Mint g(int n, int m, int x){
    Mint res = 0;
    for (int i = 0, sg = 1; i <= m / (x + 1); i++, sg *= -1){
        res += sg * C(n, i) * C(m - i * (x + 1) + n - 1, n - 1);
    }
    return res;
}
...
void Main(){
    ...
    for (int x = 1; x <= m; x++){
        ans += g(n - 1, m - x, x) * x; // a[n] = x 
        ans += g(n - 2, m - 2 * x, x) * x * (n - 1); // a[i] = x;
        Mint s = g(n - 1, m - x, x) * (m - x) / (n - 1); // a[1]
        ans -= s; 
        Mint s2 = g(n - 2, m - 2 * x, x) * (m - x); // sum(a[2],...,a[n - 1], a[n])
        ans -= s2;
    }
    cout << ans << endl; 
}
