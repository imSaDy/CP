...
struct Tree {
    ...
    vector<int> depth, start, finish, a;
    vll dp;
    vector<vector<int>> up;
    int timer;

    Tree(int n, vi &arr) : n(n), a(arr){
        LOG = 1;
        while ((1 << LOG) <= n) LOG++;
        dp.assign(n + 1, 0);
        adj.assign(n + 1, {});
...
int main(){
    ...
    tree.init();
    for (int i = 1; i <= n; i++){
        fen.range_add(tree.start[i], tree.start[i], tree.dp[i]);
    }
    while (q--){
        int t; 
        cin >> t; 
        if (t == 1){
            int s, x; 
            cin >> s >> x; 
            fen.range_add(tree.start[s], tree.finish[s], -a[s]);
            a[s] = x; 
            fen.range_add(tree.start[s], tree.finish[s], a[s]);
        }
        else {
            int s; 
            cin >> s; 
            cout << fen.point_query(tree.start[s]) << endl;
        }
    }
    return 0;
}
