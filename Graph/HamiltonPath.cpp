...
Mint count_ham_path(int n, int src, int sink, const vector<vector<int>>& g) {
    int FULL_MASK = (1 << n) - 1;
    vector<vector<Mint>> dp(FULL_MASK + 1, vector<Mint>(n, 0));
    ...
    for (int mask = 0; mask <= FULL_MASK; mask++) {
        for (int u = 0; u < n; u++) {
            ...
            for (int v : g[u + 1]) {
                int v_idx = v - 1;
                if (mask & (1 << v_idx)) continue; // already visited
                dp[mask | (1 << v_idx)][v_idx] += dp[mask][u];
            }
        }
    }

    return dp[FULL_MASK][sink - 1];
}
...
int main(){
    ...
    Mint ans = count_ham_path(n, 1, n, g);
    cout << ans; 
